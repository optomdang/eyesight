...
| 7 | BẢNG XẾP HẠNG | inTreatment; cột HOÀN THÀNH%(#8)/TẬP TRUNG%(#9)/CẢI THIỆN(số dòng xa)/PHỤC HỒI%(#10). CẢI THIỆN = test hoàn tất gần nhất − baseline; baseline ưu tiên initial trong cam kết bảo hành, nếu không có thì test full đầu tiên; lấy delta cao hơn khi cả hai mắt cải thiện. Sắp **Hoàn thành→Tập trung→Cải thiện**; Phục hồi chỉ hiển thị; LIMIT 10 | tổng hợp | tie → khóa phụ ổn định (patientId) |
| 8 | (cột) HOÀN THÀNH | **% SỐ LẦN gộp test+tập**: `(test full + lượt tập completed) / (test giao + lượt tập giao)` | [ER/ES]+[XR/XS/XC] | giao=0 → 0% |
| 9 | Tỉ lệ cải thiện theo nhóm nguyên nhân | Pie Cải thiện/Giảm sút/Ổn định, lọc `causes` (overlap). Phân loại theo **thị lực xa best-eye** `current−initial` (cùng metric BXH): >0 cải thiện, <0 giảm sút, =0 ổn định. Một mắt giảm / chỉ tương phản giảm không đẩy vào Giảm sút khi xa vẫn bằng/tốt hơn baseline. Không có far → fallback near/contrast/stereopsis | [P]+causes | thiếu init/current → bỏ |
